...
@section('content-header')
    <h1>
        {{ trans('bikers::bikers.title.edit biker') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.bikers.biker.index') }}">{{ trans('bikers::bikers.title.bikers') }}</a></li>
        <li class="active">{{ trans('bikers::bikers.title.edit biker') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.bikers.biker.update', $biker->id], 'method' => 'put','files' => 'true']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('bikers::admin.bikers.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
