...
@section('content-header')
    <h1>
        {{ trans('stores::stores.title.create stores') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.stores.stores.index') }}">{{ trans('stores::stores.title.stores') }}</a></li>
        <li class="active">{{ trans('stores::stores.title.create stores') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.stores.stores.store'], 'method' => 'post','files' => 'true']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('stores::admin.stores.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
